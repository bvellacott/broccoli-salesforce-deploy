...
const methods = {
    ...
    return new Promise((resolve, reject) => {
      if(!obj.Id) {
        ...
        .then(result => {
          ...
          log.info("successfully updated the " + type + " " + (obj.name || obj.MasterLabel));
  ...
  */
  updateOrCreate(connection, type, obj, query) {
    return new Promise((resolve, reject) => {
      var updateObj = Object.assign({}, obj);
      updateObj.AuraDefinitionBundleId = undefined;
      methods.update(connection, type, updateObj, query)
      .then(result => { return resolve(result); })
      .catch(err => {
        log.error(err);
        connection.sobject(type).create(obj)
        .then(result => {
          log.info("successfully created the " + type + " " + (obj.name || obj.MasterLabel));
          return resolve(result);
        })
        .catch(createErr => {
          log.error(createErr);
...
var deploy = {
  ...
  AuraDefinition(connection, def) {
    var bundle = {
      ...
      Description: def.description || 'no description has been provided',

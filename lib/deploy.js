...
const tooling = {
    ...
    return new Promise((resolve, reject) => {
      if(!obj.Id) {
        ...
        .execute((findErr, record) => {
          ...
          return resolve(tooling.update(connection, type, obj, query));
        });
      }
      else {
  ...
  */
  updateOrCreate(connection, type, obj, query) {
    return new Promise((resolve, reject) => {
      tooling.update(connection, type, obj, query)
...
var deploy = {
  any(connection, type, filePath, encoding, obj, contentField, query) {
    return new Promise((resolve, reject) => { 
      fs.readFile(filePath, { encoding: encoding }, (err, data) => {
        ...
        return resolve(tooling.updateOrCreate(connection, type, obj, query));
       });
    });
  },
  ...
  AuraDefinition(connection, def) {
    ...
    return tooling.updateOrCreate(connection, 'AuraDefinitionBundle', bundle, "MasterLabel = '" + def.name + "'")
    .then(result => {
      ...
      var prom = deploy.any(connection, 'AuraDefinition', def.filePath, 'utf8', definition, 'Source', 
